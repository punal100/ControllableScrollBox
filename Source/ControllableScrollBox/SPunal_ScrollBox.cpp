...
void SPunal_ScrollBox::FSlot::Construct(const FChildren& SlotOwner, FSlotArguments&& InArgs)
{
...
void SPunal_ScrollBox::FSlot::RegisterAttributes(FSlateWidgetSlotAttributeInitializer& AttributeInitializer)
{
...
SPunal_ScrollBox::FSlot::FSlotArguments SPunal_ScrollBox::Slot()
{
	return FSlot::FSlotArguments(MakeUnique<FSlot>());
}

PRAGMA_DISABLE_DEPRECATION_WARNINGS
void SPunal_ScrollPanel::Construct(const FArguments& InArgs, const TArray<SPunal_ScrollBox::FSlot*>& InSlots)
{
	PhysicalOffset = 0;
...
void SPunal_ScrollPanel::Construct(const FArguments& InArgs, TArray<SPunal_ScrollBox::FSlot::FSlotArguments> InSlots)
{
	PhysicalOffset = 0;
...
void SPunal_ScrollPanel::OnArrangeChildren(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren) const
{
...
FVector2D SPunal_ScrollPanel::ComputeDesiredSize(float) const
{
	FVector2D ThisDesiredSize = FVector2D::ZeroVector;
	for (int32 SlotIndex = 0; SlotIndex < Children.Num(); ++SlotIndex)
	{
		const SPunal_ScrollBox::FSlot& ThisSlot = Children[SlotIndex];
...
SPunal_ScrollBox::SPunal_ScrollBox()
{
	VerticalScrollBarSlot = nullptr;
	bClippingProxy = true;
}

void SPunal_ScrollBox::Construct(const FArguments& InArgs)
{
	check(InArgs._Style);
	...
	if (InArgs._ExternalScrollbar.IsValid())
	{
		...
		ScrollBar->SetOnUserScrolled(FOnUserScrolled::CreateSP(this, &SPunal_ScrollBox::ScrollBar_OnUserScrolled));
	...
	SAssignNew(ScrollPanel, SPunal_ScrollPanel, MoveTemp(const_cast<TArray<FSlot::FSlotArguments>&>(InArgs._Slots)))
...
void SPunal_ScrollBox::OnClippingChanged()
{
	ScrollPanel->SetClipping(Clipping);
}

TSharedPtr<SScrollBar> SPunal_ScrollBox::ConstructScrollBar()
{
	return TSharedPtr<SScrollBar>(SNew(SScrollBar)
		.Style(ScrollBarStyle)
		.Orientation(Orientation)
		.Padding(0.0f)
		.OnUserScrolled(this, &SPunal_ScrollBox::ScrollBar_OnUserScrolled));
}

void SPunal_ScrollBox::ConstructVerticalLayout()
{
					...
					SNew(SOverlay)
						...
						[
							...
							SNew(SImage)
								...
								.ColorAndOpacity(this, &SPunal_ScrollBox::GetStartShadowOpacity)
						...
						[
							...
							SNew(SImage)
								.Visibility(EVisibility::HitTestInvisible)
								.ColorAndOpacity(this, &SPunal_ScrollBox::GetEndShadowOpacity)
...
void SPunal_ScrollBox::ConstructHorizontalLayout()
{
...
/** Adds a slot to SPunal_ScrollBox */
SPunal_ScrollBox::FScopedWidgetSlotArguments SPunal_ScrollBox::AddSlot()
{
...
void SPunal_ScrollBox::RemoveSlot(const TSharedRef<SWidget>& WidgetToRemove)
{
	ScrollPanel->Children.Remove(WidgetToRemove);
}

void SPunal_ScrollBox::ClearChildren()
{
	ScrollPanel->Children.Empty();
}

bool SPunal_ScrollBox::IsRightClickScrolling() const
{
...
float SPunal_ScrollBox::GetScrollOffset() const
{
	return DesiredScrollOffset;
}

float SPunal_ScrollBox::GetScrollOffsetOfEnd() const
{
...
float SPunal_ScrollBox::GetViewFraction() const
{
	const FGeometry ScrollPanelGeometry = FindChildGeometry(CachedGeometry, ScrollPanel.ToSharedRef());
	const float ContentSize = GetScrollComponentFromVector(ScrollPanel->GetDesiredSize());
...
float SPunal_ScrollBox::GetViewOffsetFraction() const
{
	const FGeometry ScrollPanelGeometry = FindChildGeometry(CachedGeometry, ScrollPanel.ToSharedRef());
	const float ContentSize = GetScrollComponentFromVector(ScrollPanel->GetDesiredSize());
...
void SPunal_ScrollBox::SetScrollOffset(float NewScrollOffset)
{
...
void SPunal_ScrollBox::ScrollToStart()
{
	SetScrollOffset(0);
}

void SPunal_ScrollBox::ScrollToEnd()
{
	bScrollToEnd = true;
...
void SPunal_ScrollBox::ScrollDescendantIntoView(const TSharedPtr<SWidget>& WidgetToScrollIntoView, bool InAnimateScroll, EDescendantScrollDestination InDestination, float InScrollPadding)
{
...
bool SPunal_ScrollBox::InternalScrollDescendantIntoView(const FGeometry& MyGeometry, const TSharedPtr<SWidget>& WidgetToFind, bool InAnimateScroll, EDescendantScrollDestination InDestination, float InScrollPadding)
{
...
void SPunal_ScrollBox::SetStyle(const FScrollBoxStyle* InStyle)
{
	if (Style != InStyle)
	{
		Style = InStyle;
...
void SPunal_ScrollBox::SetScrollBarStyle(const FScrollBarStyle* InBarStyle)
{
...
void SPunal_ScrollBox::InvalidateStyle()
{
	Invalidate(EInvalidateWidgetReason::Layout);
}

void SPunal_ScrollBox::InvalidateScrollBarStyle()
{
...
EOrientation SPunal_ScrollBox::GetOrientation()
{
	return Orientation;
}

void SPunal_ScrollBox::SetNavigationDestination(const EDescendantScrollDestination NewNavigationDestination)
{
	NavigationDestination = NewNavigationDestination;
}

void SPunal_ScrollBox::SetConsumeMouseWheel(EConsumeMouseWheel NewConsumeMouseWheel)
{
	ConsumeMouseWheel = NewConsumeMouseWheel;
}

void SPunal_ScrollBox::SetOrientation(EOrientation InOrientation)
{
...
void SPunal_ScrollBox::SetScrollBarVisibility(EVisibility InVisibility)
{
	ScrollBar->SetUserVisibility(InVisibility);
}

void SPunal_ScrollBox::SetScrollBarAlwaysVisible(bool InAlwaysVisible)
{
	ScrollBar->SetScrollBarAlwaysVisible(InAlwaysVisible);
}

void SPunal_ScrollBox::SetScrollBarTrackAlwaysVisible(bool InAlwaysVisible)
{
	ScrollBar->SetScrollBarTrackAlwaysVisible(InAlwaysVisible);
}

void SPunal_ScrollBox::SetScrollBarThickness(UE::Slate::FDeprecateVector2DParameter InThickness)
{
	ScrollBar->SetThickness(InThickness);
}

void SPunal_ScrollBox::SetScrollBarPadding(const FMargin& InPadding)
{
...
void SPunal_ScrollBox::SetScrollBarRightClickDragAllowed(bool bIsAllowed)
{
	bAllowsRightClickDragScrolling = bIsAllowed;
}

EActiveTimerReturnType SPunal_ScrollBox::UpdateInertialScroll(double InCurrentTime, float InDeltaTime)
{
...
void SPunal_ScrollBox::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
...
bool SPunal_ScrollBox::ComputeVolatility() const
{
	return bIsScrolling || IsRightClickScrolling();
}

FReply SPunal_ScrollBox::OnPreviewMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FReply SPunal_ScrollBox::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FReply SPunal_ScrollBox::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	OnMouseScrolled.ExecuteIfBound(MyGeometry, MouseEvent);
...
void SPunal_ScrollBox::OnMouseEnter(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	if (MouseEvent.IsTouchEvent())
	{
		if (!bFingerOwningTouchInteraction.IsSet())
...
void SPunal_ScrollBox::OnMouseLeave(const FPointerEvent& MouseEvent)
{
	if (HasMouseCapture() == false)
	{
...
FReply SPunal_ScrollBox::OnMouseWheel(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
bool SPunal_ScrollBox::ScrollBy(const FGeometry& AllottedGeometry, float LocalScrollAmount, EAllowOverscroll Overscrolling, bool InAnimateScroll)
{
	Invalidate(EInvalidateWidget::LayoutAndVolatility);
...
FCursorReply SPunal_ScrollBox::OnCursorQuery(const FGeometry& MyGeometry, const FPointerEvent& CursorEvent) const
{
	if (IsRightClickScrolling())
	{
...
void SPunal_ScrollBox::OnMouseCaptureLost(const FCaptureLostEvent& CaptureLostEvent)
{
...
FNavigationReply SPunal_ScrollBox::OnNavigation(const FGeometry& MyGeometry, const FNavigationEvent& InNavigationEvent)
{
	...
	TPanelChildren<SPunal_ScrollBox::FSlot>& Children = ScrollPanel->Children;
	for (int32 SlotIndex = 0; SlotIndex < Children.Num(); ++SlotIndex)
	{
...
void SPunal_ScrollBox::OnFocusChanging(const FWeakWidgetPath& PreviousFocusPath, const FWidgetPath& NewWidgetPath, const FFocusEvent& InFocusEvent)
{
...
TSharedPtr<SWidget> SPunal_ScrollBox::GetKeyboardFocusableWidget(TSharedPtr<SWidget> InWidget)
{
...
int32 SPunal_ScrollBox::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
...
void SPunal_ScrollBox::ScrollBar_OnUserScrolled(float InScrollOffsetFraction)
{
	const float ContentSize = GetScrollComponentFromVector(ScrollPanel->GetDesiredSize());
	const FGeometry ScrollPanelGeometry = FindChildGeometry(CachedGeometry, ScrollPanel.ToSharedRef());
...
FSlateColor SPunal_ScrollBox::GetStartShadowOpacity() const
{
...
FSlateColor SPunal_ScrollBox::GetEndShadowOpacity() const
{
...
bool SPunal_ScrollBox::CanUseInertialScroll(float ScrollAmount) const
{
...
EAllowOverscroll SPunal_ScrollBox::GetAllowOverscroll() const
{
	return AllowOverscroll;
}

void SPunal_ScrollBox::SetAllowOverscroll(EAllowOverscroll NewAllowOverscroll)
{
...
void SPunal_ScrollBox::SetAnimateWheelScrolling(bool bInAnimateWheelScrolling)
{
	bAnimateWheelScrolling = bInAnimateWheelScrolling;
}

void SPunal_ScrollBox::SetWheelScrollMultiplier(float NewWheelScrollMultiplier)
{
	WheelScrollMultiplier = NewWheelScrollMultiplier;
}

void SPunal_ScrollBox::SetScrollWhenFocusChanges(EScrollWhenFocusChanges NewScrollWhenFocusChanges)
{
	ScrollWhenFocusChanges = NewScrollWhenFocusChanges;
}

void SPunal_ScrollBox::BeginInertialScrolling()
{
	if (!UpdateInertialScrollHandle.IsValid())
	{
		bIsScrolling = true;
		bIsScrollingActiveTimerRegistered = true;
		UpdateInertialScrollHandle = RegisterActiveTimer(0.f, FWidgetActiveTimerDelegate::CreateSP(this, &SPunal_ScrollBox::UpdateInertialScroll));
		Invalidate(EInvalidateWidget::LayoutAndVolatility);
	}
}

void SPunal_ScrollBox::EndInertialScrolling()
{
	bIsScrolling = false;
	bIsScrollingActiveTimerRegistered = false;
	Invalidate(EInvalidateWidget::LayoutAndVolatility);
