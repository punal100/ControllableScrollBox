...
// #include "SPunal_ScrollBox.generated.h"

DECLARE_DELEGATE_TwoParams(FOnMouseScroll, const FGeometry&, const FPointerEvent&);

/** SPunal_ScrollBox can scroll through an arbitrary number of widgets. */
class CONTROLLABLESCROLLBOX_API SPunal_ScrollBox : public SCompoundWidget
{
public:
	/** A Slot that provides layout options for the contents of a scrollable box. */
	class CONTROLLABLESCROLLBOX_API FSlot : public TBasicLayoutWidgetSlot<FSlot>
	{
	public:
	...
	/** @return a new slot. Slots contain children for SPunal_ScrollBox */
...
private:
	 ...
	 *
	 * @param AllottedGeometry  The geometry allotted for this SPunal_ScrollBox by the parent
	 * @param ScrollAmount
	 * @param InAnimateScroll	Whether or not to animate the scroll
...
protected:
	...
	TSharedPtr<class SPunal_ScrollPanel> ScrollPanel;
...
class SPunal_ScrollPanel : public SPanel
{
public:

	SLATE_BEGIN_ARGS(SPunal_ScrollPanel)
		{
	...
	UE_DEPRECATED(5.0, "Direct construction of FSlot is deprecated")
		void Construct(const FArguments& InArgs, const TArray<SPunal_ScrollBox::FSlot*>& InSlots);

	void Construct(const FArguments& InArgs, TArray<SPunal_ScrollBox::FSlot::FSlotArguments> InSlots);
...
public:
	...
	TPanelChildren<SPunal_ScrollBox::FSlot> Children;
